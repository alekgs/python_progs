...
Программа должна вывести дату, в которую наибольшее количество сотрудников отмечает день
рождения, в формате DD.MM.YYYY. Если таких дат несколько, программа должна вывести их все в порядке возрастания,
...
for _ in range(int(input())):
    b_day = datetime.strptime(input().rsplit(' ', 1)[1], mask)
    d[b_day] = d.get(b_day, 0) + 1

[print(i.strftime(mask)) for i in filter(lambda x: d[x] == max(d.values()), sorted(d))]

# bd_max = max(d.items(), key=lambda x: x[1])
# [print(f'{datetime.strftime(k, mask)}') for k, v in sorted(d.items()) if v == bd_max[1]]
