...
29.04.2006; 06:55 It wasn’t until several hours after launch that we were able to take the time to get out of our
seats and enter the “habitation module.” Then, after another orbital maneuver, we finally were able to take a several
hour break and get a little sleep.

03.05.2006; 20:24 Everybody had a sleeping bag although I only used mine on a couple of brief occasions,
...
# with open('diary.txt', encoding='utf-8') as file:
#     diary = {}
#     match = re.compile(r'^\d{2}.\d{2}.\d{4};\s\d{2}:\d{2}$')
#     pattern = '%d.%m.%Y; %H:%M'
#     for s in file:
#         row = []
#         if re.match(match, s.strip()):
#             date = dt.strptime(s.strip(), pattern)
#             s = file.readline()
#             while s:
#                 if s == '\n':
#                     break
#                 if s[-1:] != '\n':
#                     s += '\n'
#                 diary[date] = diary.get(date, []) + [s]
#                 s = file.readline()
#
# for k, v in sorted(diary.items()):
#     print(f'{k.strftime(pattern)}')
#     for val in v:
#         print(val, end='')
#     print()
