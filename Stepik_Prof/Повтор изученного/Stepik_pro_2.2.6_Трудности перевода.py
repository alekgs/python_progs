...
режиссер. В каждой из следующих n строк через запятую и пробел указывается список языков, которые знает человек.
...
"""
langs = [set(input().split(', ')) for _ in range(int(input()))]
result = set(langs[0])
for s in langs:
    result &= s

print(('Сериал снять не удастся', ', '.join(sorted(result)))[len(result) > 0])
