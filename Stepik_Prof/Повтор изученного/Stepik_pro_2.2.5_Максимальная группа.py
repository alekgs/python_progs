...
nums, sl = [n for n in range(1, int(input('Введите длину последовательности n: ')) + 1)], {}
for k in nums:
    s_num = sum(map(int, str(k)))
    sl[s_num] = sl.get(s_num, 0) + 1
print(f'Сумма чисел и количество этих сумм в последовательности:\n{sl}')
print(f'Макс. длина группы: {max(sl.values())}')
