...
Примечание 1.
Начальная часть ответа выглядит так (в качестве отступов используйте четыре пробела):
...
f = lambda x: x[0]
sorted_tasks = sorted(tasks, key=f)

for k, v in groupby(sorted_tasks, key=f):
    print(f'{k}:')
    for _, task, number in sorted(v, key=lambda x: x[2]):
        print(f'\t{number}. {task}')
    print()
