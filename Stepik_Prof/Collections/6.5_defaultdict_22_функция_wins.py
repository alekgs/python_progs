...
def wins(pairs: list) -> dict:
    """
    Возвращает словарь, в котором ключом служит имя ученика, а значением
    — множество (тип set) имен учеников, которых он победил
    :param pairs: список кортежей из имён (победитель, проигравший)
    :return: res: результат
    """

    res = defaultdict(set)
    [res[win].add(los) for win, los in pairs]
    return res
