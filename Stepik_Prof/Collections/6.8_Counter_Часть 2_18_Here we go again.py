...
каждого соответствующее количество смененных имен.
...
with open('name_log.csv', encoding='utf-8') as f:
    count_email = Counter([s['email'] for s in csv.DictReader(f)])
    [print(f'{email}: {count}') for email, count in sorted(count_email.items())]
