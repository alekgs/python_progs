...
Программа должна вывести введенный блок кода, предварительно удалив из него все строки которые
содержат в себе только комментарии.
...
[stdout.write(s) for s in stdin if not s.strip().startswith('#')]

# 2
# print(sum(map(lambda x: x.lstrip().startswith('#'), open(0))))
