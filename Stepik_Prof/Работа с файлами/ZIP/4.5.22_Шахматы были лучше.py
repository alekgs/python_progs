...
Примечание 1.
Обратите внимание, что наличие у файла расширения .json не гарантирует, что он является корректным
...
with ZipFile("data.zip") as zf:
    ...
    for file in zf.infolist():
        if file.filename.endswith(".json"):
            try:
                data = json.loads(zf.read(file).decode('utf-8', 'ignore'))
                if 'Arsenal' in data['team']:
                    result.append(f"{data['first_name']} {data['last_name']}")
            except ValueError:
                continue

print(*sorted(result, key=lambda x: (x.split()[0], x.split()[1])), sep='\n')
