...
with open('food_services.json', encoding='utf-8') as f_in:
    data, res1 = __import__('json').load(f_in), {}
    for s in data:
        res1[s['TypeObject']] = res1.get(s['TypeObject'], []) + [(s['Name'], s['SeatsCount'])]

for type_obj, name_sc in sorted(res1.items()):
    name, seats_count = max(name_sc, key=lambda x: x[1])
