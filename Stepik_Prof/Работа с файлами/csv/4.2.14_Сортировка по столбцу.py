...
#
# n_col = int(input()) - 1
# rows.sort(key=lambda x: int(x[n_col]) if x[n_col].isdigit() else x[n_col])
# [print(*lst, sep=',') for lst in rows]

# pandas
from sys import stdout
import pandas as pd

df = pd.read_csv('deniro.csv', encoding='UTF-8', header=None)
df = df.sort_values(by=df.columns[int(input()) - 1], kind='stable')
df.to_csv(stdout, header=False, index=False)
