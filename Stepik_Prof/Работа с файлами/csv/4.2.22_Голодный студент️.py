...
фасоль;Мороженое;Фарш;Вареники;Картофель;Батончик Пятерочка;69;133;129;83;141;90;72;123;149;89;88;106;54;84
...
"""
# import pandas as pd
# st = pd.read_csv('prices.csv', sep=';').set_index('Магазин')
# shop, product = st.stack().idxmin()
# print(f'{product}: {shop}')

import csv

with open('prices.csv', encoding='utf-8') as file:
    rows, res = csv.DictReader(file, delimiter=';'), {}
    columns = rows.fieldnames
    for row in file:
        shop, *prod = row.strip().split(';')
        res[shop] = min(zip(columns[1:], map(int, prod)), key=lambda x: x[1])
        magazine, product = min(res.items(), key=lambda x: (x[1][1], x[1][0]))
print(f'{product[0]}: {magazine}')
