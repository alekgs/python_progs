...
период 20002000 — 20212021 г. В первом столбце записан год, в последующих столбцах записан класс и количество
...
# import pandas as pd
#
# df = pd.read_csv('student_counts.csv').set_index('year')
# df = df[sorted(df.columns, key=lambda x: (int(x.split('-')[0]), x.split('-')[1]))]
# df.to_csv('sorted_student_counts.csv')
