...
Первый список содержит названия различных мультфильмов, второй и третий — соответствующие бюджеты и сборы в долларах.
...
[print(f'{f}: {bo - bd}$') for f, bd, bo in sorted(zip(names, budgets, box_offices))]

# res = {f: bo - bd for f, bd, bo in zip(names, budgets, box_offices)}
# [print(f'{k}: {v}$') for k, v in sorted(res.items())]


# import pandas as pd
# df = pd.DataFrame([names, budgets, box_offices]).T
# df.columns = ['Film', 'Budget', 'Box_office']
# df['Profit'] = df['Box_office'] - df['Budget']
# print(df[['Film', 'Profit']].sort_values('Film'))
